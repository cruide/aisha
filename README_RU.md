...
[llm]
temperature = 0.6           # по умолчанию 0.3; 0.0 .. 2.0
top_p = 0.9                 # по умолчанию 0.9; 0.0 .. 1.0
top_k = 40                  # по умолчанию 30; целое > 0
repeat_penalty = 1.1        # по умолчанию 1.03; > 0
frequency_penalty = 0.0     # по умолчанию 0; -2.0 .. 2.0
max_output_tokens = 32768
context_window = 32768
context_soft_limit = 0.75
max_tool_iterations = 25
tool_guide = true            # добавить «Справочник инструментов» в системный промпт; отключайте только для экономии контекста
...
- **Память** — Markdown в `~/.aisha/memory/MEMORY.md` (глобально) и
  `<workspace>/.aisha/memory/MEMORY.md` (проектно). Каждый блок — секция `## label`
  с необязательными комментариями `<!-- description: … -->` / `<!-- updated_at: … -->`.
  Проектный блок перекрывает глобальный с тем же `label`.
  `memory_set.scope` по умолчанию равен `global`; для памяти workspace укажите `project`.
  Старые `*.json`-блоки импортируются один раз при запуске и затем удаляются.
...
При запуске баннер показывает версию, модель, размер контекста, workspace, адрес сервера,
режим, оболочку и действующие параметры сэмплинга (`temperature`, `top_p`, `top_k`,
`repeat_penalty`). Значения из конфига выводятся как есть; параметр, который не задан,
берётся из серверных настроек и показывается как `server`.
